...
**スクリプト読み込み順の罠（`popup.html`）**: `popup.js` を必ず先に読み込む。モジュールの `html` テンプレートリテラルが**読み込み時点で** `Toolkit.copyButton` / `iconButton` / `ICONS` を呼ぶため、順序を誤ると未定義で落ちる。また **UI のタブ順 = `popup.html` のスクリプト順**。モジュール追加 = `modules/<name>.js` 作成 ＋ `popup.html` に `<script>` 追加。
