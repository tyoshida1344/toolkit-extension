 ...
 * ポップアップ(index.js)とバー(bar.js)が共有。
 *
 * 注意: executeScript は run だけを func.toString() で直列化して注入する。よって
 * run が参照するもの（build/create とそのヘルパー）は**すべて run の内側**に置き、
 * 外側スコープを一切参照しないこと（外に出すと「... is not defined」で落ちる）。
 */
window.SiteSearchEngine = (() => {
  ...
  function run(action, query, o) {
    ...
    function build(q, opt) {
      return {
        pattern: opt.regexMode === false ? q.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') : q,
        flags: 'g' + (opt.caseSensitive ? '' : 'i') + 'm',
      };
    }
    ...
    function create() {
      ...
      return {
        ...
        search(q, opt) {
          clear();
          const { pattern, flags } = build(q, opt);
          const r = find(pattern, flags, opt.max || 2000);
          ...
          if (ranges.length) {
            ...
            setCurrent(opt.startIdx > 0 ? opt.startIdx : 0);
          }
        ...
        count(q, opt) {
          const { pattern, flags } = build(q, opt);
          const r = find(pattern, flags, opt.max || 300);
          if (r.error) return { ok: false, error: r.error, message: r.message };
